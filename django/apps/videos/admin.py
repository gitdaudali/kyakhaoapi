...
class VideoAdmin(admin.ModelAdmin):
    ...
    list_filter = ["status", "uploaded_at", "format", "uploaded_by__role"]
    search_fields = [
        "title",
        "description",
        "uploaded_by__email",
    ...
    fieldsets = (
        ...
        ("Media URLs", {"fields": ("thumbnail_url",)}),
        (
            "S3 Information",
            {"fields": ("s3_bucket", "s3_key"), "classes": ("collapse",)},
        ),
        ("Statistics", {"fields": ("views_count", "likes_count", "status")}),
        (
            "Timestamps",
    ...
    def file_url_link(self, obj):
        ...
        if obj.file_url:
            return format_html(
                '<a href="{}" target="_blank">{}</a>', obj.file_url, "View File"
            )
        return "-"
