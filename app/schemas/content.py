...
# =============================================================================
# STREAMING DECRYPTION SCHEMAS
# =============================================================================


class PlayContentRequest(BaseModel):
    """Request schema for playing content with streaming decryption"""

    quality: str = Field("HD", description="Video quality (4K, HD, SD)")
    season_number: Optional[int] = Field(
        None, description="Season number (for TV series)"
    )
    episode_number: Optional[int] = Field(
        None, description="Episode number (for TV series)"
    )


class ChunkRequest(BaseModel):
    """Request schema for getting a streaming chunk"""

    chunk_index: int = Field(..., description="Chunk index (0-based)")
    access_token: str = Field(..., description="Access token from /play endpoint")


class LicenseRequest(BaseModel):
    """Request schema for getting decryption license"""

    access_token: str = Field(..., description="Access token from /play endpoint")


class ProcessingStatusResponse(BaseModel):
    """Response when content is being processed"""
...
class NotEncryptedResponse(BaseModel):
    """Response when content is not encrypted yet"""

    content_id: UUID = Field(..., description="Content ID")
    title: str = Field(..., description="Content title")
    status: str = Field("not_encrypted", description="Status")
    message: str = Field(..., description="Status message")
    file_url: str = Field(..., description="Original file URL (temporary)")
    is_encrypted: bool = Field(False, description="Whether content is encrypted")


class PlayContentResponse(BaseModel):
    """Response for playing encrypted content with streaming decryption"""
    ...
    status: str = Field("ready", description="Playback status")

    # Streaming Information
    streaming_url: str = Field(..., description="Base streaming URL for chunks")
    access_token: str = Field(..., description="Access token for streaming")
    chunk_size: int = Field(1048576, description="Chunk size in bytes (1MB)")
    total_file_size: int = Field(..., description="Total file size in bytes")
    total_chunks: int = Field(..., description="Total number of chunks")
    encryption_method: str = Field("AES-256-CBC", description="Encryption method")

    # Metadata
    duration_seconds: float = Field(..., description="Video duration")
    bitrate_kbps: Optional[int] = Field(None, description="Bitrate")
    video_codec: Optional[str] = Field(None, description="Video codec")
    audio_codec: Optional[str] = Field(None, description="Audio codec")
    container_format: Optional[str] = Field(None, description="Container format")
...
class ChunkResponse(BaseModel):
    ...
    total_chunks: int = Field(..., description="Total number of chunks")
    chunk_data: str = Field(..., description="Encrypted chunk data (base64)")
    decryption_key: str = Field(..., description="Decryption key for this chunk")
    iv: str = Field(..., description="Initialization vector")
    is_last_chunk: bool = Field(False, description="Whether this is the last chunk")


class LicenseResponse(BaseModel):
    """Response for decryption license"""

    key_id: str = Field(..., description="Encryption key ID")
    file_id: str = Field(..., description="File ID")
    decryption_key: str = Field(..., description="Decryption key (base64)")
    usage_count: int = Field(..., description="Current usage count")
    max_usage: Optional[int] = Field(None, description="Maximum usage limit")
    expires_at: Optional[datetime] = Field(None, description="Key expiration time")
    token_expires_at: str = Field(..., description="Token expiration time")


class FileEncryptionStatus(BaseModel):
    """Encryption status for a single file"""

    file_id: UUID = Field(..., description="File ID")
    file_type: str = Field(..., description="File type (movie_file, episode_quality)")
    quality: str = Field(..., description="Video quality")
    season_number: Optional[int] = Field(None, description="Season number")
    episode_number: Optional[int] = Field(None, description="Episode number")
...
class EncryptionStatusResponse(BaseModel):
    ...
    title: str = Field(..., description="Content title")
    content_type: str = Field(..., description="Content type")
    overall_status: str = Field(..., description="Overall encryption status")
    total_files: int = Field(..., description="Total number of files")
    encrypted_files: int = Field(..., description="Number of encrypted files")
    processing_files: int = Field(..., description="Number of files being processed")
    failed_files: int = Field(..., description="Number of failed files")
    files: List[FileEncryptionStatus] = Field(
        ..., description="Individual file statuses"
    )
